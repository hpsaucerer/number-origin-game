...
// read a local "extra completions" offset (always readable; local only)
...
// write/clear offsets — always act locally so preview works even if env var is missing
...
// Base path for badges (swap to CDN/Storage by setting env)
export const BADGES_BASE =
  process.env.NEXT_PUBLIC_BADGES_BASE_URL || "/badges";

// canonical label -> filename slug
const CATEGORY_SLUG = {
  Maths: "maths",
  Geography: "geography",
  Science: "science",
...
/**
 * Build the correct badge URL.
 * - Tier 20 uses your category artwork: /badges/<slug>-20.png
 * - Tiers 50/100 fall back to /badges/default-50.png or /badges/default-100.png
 *   (drop in <slug>-50.png / <slug>-100.png later without code changes)
 */
export function badgeUrlFor(category, tier, ext = "png") {
  const canon = normaliseCategory(category) || category;
  const slug = CATEGORY_SLUG[canon];
  if (tier === 20 && slug) {
    return `${BADGES_BASE}/${slug}-20.${ext}`;
  }
  return `${BADGES_BASE}/default-${tier}.${ext}`;
}
